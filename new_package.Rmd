...
In my case, I'll use tools from the tidyverse package for this function, so I'll add that dependancy in this feature branch, too (similar to how I added GWASTools before). After some work, here's the commit that merges `R/import-gencode.r` and `tests/testthat/test_import_gencode.r` from the *features/gen_import* branch back to the *develop* branch - so you can see where the code is at that point.
